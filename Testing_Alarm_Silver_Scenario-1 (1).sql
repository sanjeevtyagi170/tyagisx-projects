...
CREATE OR REFRESH STREAMING LIVE TABLE alarm_test
(
  --CONSTRAINT valid_date_time_notif EXPECT(CAST(date_time_notif AS TIMESTAMP) IS NOT NULL) ON VIOLATION DROP ROW
  CONSTRAINT valid_date_time_notif EXPECT(DATE_TIME_NOTIF RLIKE ('^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}.[0-9]{6}'))
)
AS
SELECT 
  ID_ALARM,
